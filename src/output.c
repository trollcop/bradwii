...
void setoutput(unsigned char outputchannel, unsigned int value)
{
    ...
    if (timernum == OUTPUT_TIMER1) {
        if (timerchannel == OUTPUT_CHANNELA)
            lib_pwm_setduty1A(pwmvalue);
        else if (timerchannel == OUTPUT_CHANNELB)
            lib_pwm_setduty1B(pwmvalue);
        return;
    }
#endif
#ifdef USEPWM2
    if (timernum == OUTPUT_TIMER2) {    // timer 2 only has an 8 bit range
...
#endif
        if (timerchannel == OUTPUT_CHANNELA)
            lib_pwm_setduty2A(pwmvalue);
        else if (timerchannel == OUTPUT_CHANNELB)
            lib_pwm_setduty2B(pwmvalue);
        return;
    }
#endif
#ifdef USEPWM3
    if (timernum == OUTPUT_TIMER3) {
        if (timerchannel == OUTPUT_CHANNELA)
            lib_pwm_setduty3A(pwmvalue);
        else if (timerchannel == OUTPUT_CHANNELB)
            lib_pwm_setduty3B(pwmvalue);
        else if (timerchannel == OUTPUT_CHANNELC)
            lib_pwm_setduty3C(pwmvalue);
        return;
    }
#endif
#ifdef USEPWM4
    if (timernum == OUTPUT_TIMER4) {
        if (timerchannel == OUTPUT_CHANNELA)
            lib_pwm_setduty4A(pwmvalue);
        else if (timerchannel == OUTPUT_CHANNELB)
            lib_pwm_setduty4B(pwmvalue);
        else if (timerchannel == OUTPUT_CHANNELC)
            lib_pwm_setduty4C(pwmvalue);
        return;
    }
#endif
#ifdef USEPWM411BIT
    if (timernum == OUTPUT_TIMER4) {
        ...
        if (timerchannel == OUTPUT_CHANNELA)
            ...
            lib_pwm_setduty4B(pwmvalue);
        else if (timerchannel == OUTPUT_CHANNELD)
            lib_pwm_setduty4D(pwmvalue);
        return;
    }
#endif
    ...
    if (timernum == OUTPUT_TIMER5) {
        if (timerchannel == OUTPUT_CHANNELA)
            lib_pwm_setduty5A(pwmvalue);
        else if (timerchannel == OUTPUT_CHANNELB)
            lib_pwm_setduty5B(pwmvalue);
        else if (timerchannel == OUTPUT_CHANNELC)
            lib_pwm_setduty5C(pwmvalue);
        return;
    }
#endif
}
