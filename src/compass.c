...
void compassreadrawvalues(int *compassrawvalues)
{
    ...
    COMPASS_ORIENTATION(compassrawvalues, ((data[0] << 8) | data[1]), ((data[2] << 8) | data[3]), ((data[4] << 8) | data[5]));
#endif
#if (COMPASS_TYPE==HMC5883)
    COMPASS_ORIENTATION(compassrawvalues, ((data[0] << 8) | data[1]), ((data[4] << 8) | data[5]), ((data[2] << 8) | data[3]));
#endif
...
void initcompass()
{
    ...
    lib_i2c_writereg(MPU6050_ADDRESS, 0x6A, 0 b00100000);       //USER_CTRL     -- DMP_EN=0 ; FIFO_EN=0 ; I2C_MST_EN=1 (I2C master mode) ; I2C_IF_DIS=0 ; FIFO_RESET=0 ; I2C_MST_RESET=0 ; SIG_COND_RESET=0
...
void calibratecompass()
{
    ...
    while (lib_timers_gettimermicroseconds(testtimer) < 30000000L) {
        ...
        for (int x = 0; x < 3; ++x) {
            if (compassfilteredrawvalues[x] < minvalues[x])
                minvalues[x] = compassfilteredrawvalues[x];
            if (compassfilteredrawvalues[x] > maxvalues[x])
                maxvalues[x] = compassfilteredrawvalues[x];
        }
    }
